...
router.post("/login", async (req, res) => {
    try{
        ...
        res.status(200).json(others); //if the login credentials are all correct, it sends all the json "other" user object to console, except the hashed password (for security). So the password doesnt show even though it is there.
    } 
    catch(err) {
